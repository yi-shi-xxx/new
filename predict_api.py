...
def serve_announcements_html():
    return send_from_directory('.', 'announcements.html')

@app.route('/cases.html')
def serve_cases_html():
    return send_from_directory('.', 'cases.html')

@app.route('/data_upload.html')
def serve_data_upload_html():
    return send_from_directory('.', 'data_upload.html')\

@app.route('/feedback.html')
def serve_feedback_html():
    return send_from_directory('.', 'feedback.html')\

@app.route('/help_center.html')
def serve_help_center_html():
    return send_from_directory('.', 'help_center.html')

@app.route('/login.html')
def serve_login_html():
    return send_from_directory('.', 'login.html')

@app.route('/model_introduction.html')
def serve_model_introduction_html():
    return send_from_directory('.', 'model_introduction.html')\
...
def serve_user_profile_html():
    return send_from_directory('.', 'user_profile.html')
...
def predict_batch():
    try:
        ...
        X = df.values
        oer_preds = oer_model.predict(X)
        orr_preds = orr_model.predict(X)
        results = []
        for oer_pred, orr_pred in zip(oer_preds, orr_preds):
            # 自动建议：敏感性分析
            step = 0.05  # 5%微调
            oer_delta = {}
            orr_delta = {}
            for i, fname in enumerate(feature_names):
                # OER
                new_features_up = X.copy()
                new_features_down = X.copy()
                new_features_up[:, i] *= (1 + step)
                new_features_down[:, i] *= (1 - step)
                oer_up = oer_model.predict(new_features_up)
                oer_down = oer_model.predict(new_features_down)
                if oer_up < oer_pred:
                    oer_delta[fname + "↑"] = oer_pred - oer_up
                if oer_down < oer_pred:
                    oer_delta[fname + "↓"] = oer_pred - oer_down
                # ORR
                orr_up = orr_model.predict(new_features_up)
                orr_down = orr_model.predict(new_features_down)
                if orr_up < orr_pred:
                    orr_delta[fname + "↑"] = orr_pred - orr_up
                if orr_down < orr_pred:
                    orr_delta[fname + "↓"] = orr_pred - orr_down
            ...
            results.append({'OER': float(oer_pred), 'ORR': float(orr_pred), '建议': advice})
